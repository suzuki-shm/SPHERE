functions{
    ...
    real wrappedcauchy_mixture_lpdf(real R, int K, vector a, vector mu, vector rho, int L) {
        vector[K] lp;
        for (k in 1:K){
            lp[k] = log(a[k]) + wrappedcauchy_lpdf(R | mu[k], rho[k]) + log(2.0) + log(pi()) - log(L) ;
        }
...
model {
    ...
    for(i in 1:I){
        target += DEPTH[i] * wrappedcauchy_mixture_lpdf(RADIAN[i] | K, alpha, ori, rho[SUBJECT[i]], L) ;
    }
}
...
generated quantities {
    ...
    for(i in 1:I){
        log_lik[i] = DEPTH[i] * wrappedcauchy_mixture_lpdf(RADIAN[i] | K, alpha, ori, rho[SUBJECT[i]], L) ;
    }
