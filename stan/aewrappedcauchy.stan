functions{
    ...
    real aewrappedcauchy_mixture_lpdf(real R, int K, vector a, vector mu, vector kappa, vector nu) {
        vector[K] lp ;
        for (k in 1:K){
            lp[k] = log(a[k]) + aewrappedcauchy_lpdf(R | mu[k], kappa[k], nu[k]) ;
        }
...
model {
    ...
    for(i in 1:I){
        target += DEPTH[i] * aewrappedcauchy_mixture_lpdf(RADIAN[i] | K, alpha, ori, kappa[SUBJECT[i]], nu[SUBJECT[i]]) ;
    }
}
...
generated quantities {
    ...
    for(i in 1:I){
        log_lik[i] = DEPTH[i] * aewrappedcauchy_mixture_lpdf(RADIAN[i] | K, alpha, ori, kappa[SUBJECT[i]], nu[SUBJECT[i]]) ;
    }
}
