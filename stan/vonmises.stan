...
model {
    ...
    for(i in 1:I){
        target += DEPTH[i] * von_mises_mixture_lpdf(RADIAN[i] | K, alpha, ori, kappa[SUBJECT[i]], L) ;
    }
}
...
generated quantities {
    ...
    for(i in 1:I){
        log_lik[i] = DEPTH[i] * von_mises_mixture_lpdf(RADIAN[i] | K, alpha, ori, kappa[SUBJECT[i]], L) ;
    }
